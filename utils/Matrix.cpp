...
// --------------------------------------------------------------------------
// Decomposição LU com pivotamento parcial (Doolittle)
// --------------------------------------------------------------------------
bool Matrix::luDecompose(std::vector<int>& piv) {
    ...
    for (int k = 0; k < n; ++k) {
        // Pivotamento parcial: encontra o maior elemento na coluna k
        int maxRow = k;
...
// --------------------------------------------------------------------------
// Resolução Ax = b após LU (forward/backward substitution)
// --------------------------------------------------------------------------
...
std::vector<double> Matrix::normalize(const std::vector<double>& v) {
    double n = vecNorm(v);
    if (n < 1e-14) throw std::runtime_error("normalize — vetor nulo.");
    std::vector<double> u(v.size());
    for (size_t i = 0; i < v.size(); ++i) u[i] = v[i] / n;
    return u;
}
